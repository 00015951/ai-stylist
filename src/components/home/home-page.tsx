...
export function HomePage() {
  ...
  const handleSend = async (text?: string) => {
    ...
    const doEdit =
      ...
      (text ?? inputValue).trim() !== editFromResult.occasion;

    if (doEdit) {
      try {
        await new Promise((r) => setTimeout(r, 1200));
        const editInstructions = (text ?? inputValue).trim();
        const fakeResult: GeneratedStyleResult = {
          occasion: occasionText,
          personaSummary: `Edited look: ${editInstructions}. Light tweaks for your occasion.`,
          outfit: {
            top: "Edited: Relaxed linen shirt",
          ...
          outfits: [
            {
              ...
              whyItWorks: `Edited look: ${editInstructions}. Light tweaks for your occasion.`,
            },
          ],
        };
        setMessages((prev) => [
          ...
          {
            role: "assistant",
            content: fakeResult.personaSummary,
            result: fakeResult,
          },
        ]);
        setLastGeneratedResult(fakeResult);
